...
const ArticleCard = (props: ArticleCardProps) => {
    const {article, handleSetSingleArticle, loginUserId, handleUpdateArticle, handleDeleteArticle} = props;
    const [animation, setAnimation] = useState<object>({});
    const [transition, setTransition] = useState<object>({});

    return (
        <motion.li initial={{scale: 1}} transition={transition} animate={animation} exit={{scale: '100%'}} className='comp-articleCard'>
